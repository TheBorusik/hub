...
export function RolesPanel() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setRoles(res.Roles as unknown as Role[]);
        } catch {
            setRoles([]);
        } finally {
            setLoading(false);
    ...
    const loadPerms = useCallback(async (roleId: number) => {
        ...
        try {
            const res = await api.getRolePermissions({RoleId: roleId});
    ...
    const handleDeleteRole = async (role: Role) => {
        ...
        if (!ok) return;
        await api.removeRole({RoleId: role.RoleId});
    ...
    const handleTogglePermAction = async (perm: RolePermission) => {
        ...
        if (perm.Action === "Allow") {
            await api.denyPermissionsForRole({RoleId: selectedRoleId, PermissionIds: [perm.PermissionId]});
        } else {
            await api.assignPermissionsToRole({RoleId: selectedRoleId, PermissionIds: [perm.PermissionId]});
        }
    ...
    const handleRemovePerm = async (permId: number) => {
        if (!api || selectedRoleId == null) return;
        await api.removePermissionsFromRole({RoleId: selectedRoleId, PermissionIds: [permId]});
        loadPerms(selectedRoleId);
    };
    ...
    return (
        <div ref={containerRef} className="flex h-full overflow-hidden" style={{position: "relative"}}>
            ...
            <div className="flex flex-col shrink-0"
                 style={{width: leftWidth, borderRight: `1px solid ${tok.color.border.default}`}}>
                <PanelToolbar
                    dense
                    left={
                        <>
            ...
            <div
                ...
                onMouseEnter={(e) => {
                    e.currentTarget.style.background = "rgba(14,99,156,0.5)";
                }}
                onMouseLeave={(e) => {
            ...
            {overlay.type === "upsertRole" && api && (
                <RoleDialog editing={overlay.editing} api={api} onClose={() => setOverlay({type: "none"})}
                            onDone={() => {
