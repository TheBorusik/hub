...
export function TablesPanel() {
    ...
    const loadTables = useCallback(async () => {
        ...
        try {
            ...
            if (Array.isArray(list)) {
                setTables(res.Tables.filter(x => x.Name));
            }
        } catch {
            setTables([]);
        }
    ...
    const loadData = useCallback(async (tableName: string) => {
        ...
        setLoading(true);
        try {
            const metaRes = await api.getTableMeta({TableName: tableName});
            const fieldInfos = metaRes.FieldInfos;
            const parsedMeta: FieldInfo[] = Array.isArray(fieldInfos)
                ? (fieldInfos as unknown as FieldInfo[])
                : [];
            setMeta(parsedMeta);

            const dataRes = await api.getTableData({TableName: tableName});
    ...
    useEffect(() => {
        if (selected) loadData(selected); else {
            setMeta([]);
            setData([]);
        }
    }, [selected, loadData]);
    ...
    const handleDelete = async (row: Record<string, unknown>) => {
        ...
        if (!ok) return;
        await api.deleteTableData({TableName: selected, Data: [row]});
        loadData(selected);
    };
    ...
    const handleUpsert = async (json: string) => {
        ...
        await api.upsertTableData({TableName: selected, Data: [parsed]});
                    ...
                    <>
                        {selected && (
                            ...
                            >
                                ...
                                />
                                {filter && (
                                    <IconButton size="xs" label="Clear filter" icon={<X size={10}/>}
                                                onClick={() => setFilter("")}/>
                                )}
                            </div>
                        )}
