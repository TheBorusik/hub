...
import type {WebProcess, DiagnosticModel, WfmProcessAssemblyUpsertResponse} from "@/lib/ws-api-models";
...
export function useProcessSave({
    ...
    const handleSave = useCallback(async () => {
        ...
        const tasks: Array<Promise<unknown>> = [
            api.upsertProcessAssembly({
                Name: process.TypeName,
                Category: "PROCESS",
                Model: process,
                CreateNew: false,
            }),
        ...
        if (hasWebData) {
            tasks.push(
                api.upsertProcessAssembly({
                    Name: process.TypeName + "WebData",
                    Category: "WEBDATA",
                    Model: process.WebData,
                    CreateNew: false,
                }),
            );
        }
        ...
        if (processOk) {
            const payload = (processRes as PromiseFulfilledResult<WfmProcessAssemblyUpsertResponse>).value;
    ...
    const handleValidateProcess = useCallback(async () => {
        ...
        setValidating(true);
        try {
            const res = await api.validateProcess({Process: process});
