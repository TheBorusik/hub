...
interface ProcessDiagramProps {
    ...
    /** Найти использования зависимости (CRUD/Command/Event/Sub). */
...
function buildNodes(process: WebProcess, callbacks?: BuildNodesCallbacks): Node<StageNodeData>[] {
    ...
    return Object.keys(stages).map((name, idx) => {
        ...
        // В Properties для SubStart сохраняется ProcessTypeName (не ProcessName).
        const subProcessName = (stage.Properties?.ProcessTypeName ?? "").trim() || undefined;

        // Извлекаем имя зависимости для контекстного меню "Find Usages"
        let dependencyName: string | undefined;
        let dependencyType: string | undefined;
        switch (stage.Type) {
            case "CRUD":
                dependencyName = stage.Properties?.Model;
                dependencyType = "crud";
                break;
            case "Command":
                dependencyName = stage.Properties?.CommandName;
                dependencyType = "commands";
                break;
            case "Event":
                dependencyName = stage.Properties?.EventName;
                dependencyType = "events";
                break;
            case "SubStart":
                dependencyName = stage.Properties?.ProcessTypeName ?? subProcessName;
                dependencyType = "subs";
                break;
        }
...
function DiagramInner({
    ...
    const addNewStage = useCallback((type: string, name: string) => {
        ...
        const startStage = Object.values(process.Stages ?? {}).find(x => x.Type === "Start");

        const startWebData = process.WebData.Stages[startStage!.Name].Position;

        const newWebStage = {
            Position: {x: startWebData.x + 200, y: startWebData.y},
    ...
    const nodeCallbacks = useMemo<BuildNodesCallbacks>(() => ({
        ...
        onOpenSubProcess,
        onFindUsages,
    }), [onSelectStage, handleSetStartup, onOpenSubProcess, askDeleteStage, onFindUsages]);
    ...
    useEffect(() => {
        setNodes(buildNodes(process, nodeCallbacks));
    }, [process, nodeCallbacks]);
    useEffect(() => {
        ...
        (changes: NodeChange<Node<StageNodeData>>[]) => {
            ...
            if (drags.length > 0) {
                ...
                for (const ch of drags) {
                    ...
                    upd[ch.id] = {
                        ...(upd[ch.id] ?? {Position: {x: 0, y: 0}, Color: "#888", Lines: {}}),
                        Position: {x: Math.round(ch.position.x), y: Math.round(ch.position.y)}
                    };
                }
                onProcessUpdate({
                    ...process,
                    WebData: process.WebData ? {...process.WebData, Stages: upd} : {Stages: upd}
                });
            }
        },
        [process, onProcessUpdate],
